...
import { Loader2, Eye, EyeOff, UserCircle, KeyRound, CheckCircle2, Phone, ShieldCheck, ArrowLeft, MessageSquare, UserPlus } from "lucide-react";
...
export default function EmployeeLogin() {
  ...
  const { isExpired: isOtpExpired, seconds: otpSeconds } = useCountdown(otpExpiryDate);
  ...
  async function handleRequestOtp(e?: React.FormEvent) {
    ...
    setBusy(true);
    try {
      // Re-using the same forgot-password request OTP since Employees are in Employee model,
        ...
        <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-2xl shadow-lg border border-slate-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label className="text-sm font-semibold text-slate-700 mb-1">Department</Label>
                <Select 
                  value={department} 
                  onValueChange={(val) => {
                    setDepartment(val);
                    setSubDepartment("none"); // Reset sub-department when department changes
                  }} 
                  required
                >
              ...
              <div>
                <Label className="text-sm font-semibold text-slate-700 mb-1">Sub Department</Label>
                <Select value={subDepartment} onValueChange={setSubDepartment} disabled={availableSubDepts.length === 0}>
                  <SelectTrigger className="w-full h-10 bg-white">
          ...
          <div className="pt-4 border-t border-slate-100 space-y-4 text-center">
            <div className="text-sm">
              <span className="text-slate-500">New Employee? </span>
              <Link href="/employee/register" className="font-semibold text-primary hover:underline inline-flex items-center gap-1">
                ...
                <form onSubmit={handleRequestOtp} className="space-y-4 pt-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="employee@domain.com or 9876543210"
                      required
                    />
                  </div>
